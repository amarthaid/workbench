...
- [2026-09-10 browser session pod affinity](docs/findings/2026-09-10-browser-session-pod-affinity.md) — the per-user chromium, `warmSessions`, `activeProfiles` and the live-view `channels` are all module-level maps, so every browser request must reach the owning process (a *worker* under `CLUSTER_ENABLED`); capture and `browser_*` always needed this, and a second process on a shared profiles volume deletes the `SingletonLock` a live chromium holds; the SSE bridge turned one routing decision into four, so a misrouted `commands` POST 404s — now recovered by re-attaching instead of silently dropping input
