...
- [2026-09-17 custom apps](docs/findings/2026-09-17-custom-apps.md) — per-user external MCP servers (HTTP streamable, OAuth 2.1) modeled as Apps: dynamic client registration from a base URL, per-user tool index parallel to the static plugin registry (`appName__tool`), no local arg validation (Claude Desktop parity), tokens reuse `connections` under `custom:<id>`; `encryption.ts` computes its key lazily because a load-time `config` read broke config-mocked tests
