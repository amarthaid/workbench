...
- [2026-09-13 stateless jot upload token](docs/findings/2026-09-13-stateless-jot-upload-token.md) — jot upload tokens lived in a module-level Map, so a restart voided them and `CLUSTER_ENABLED` broke uploads ~(N-1)/N of the time with a bare 404; now an encrypted JWT (JWE `dir`+A256GCM, HKDF off `SESSION_SECRET`) — encrypted not signed because the claims carry the owner id and a password jot's scrypt hash. Cost: find-my-way's 100-char `maxParamLength` rejects a ~320-char path param with 414 (set it from `MAX_TOKEN_CHARS` under Fastify 5's `routerOptions`), `deletes` is now URL-bounded (`TOO_MANY_DELETES`), and single-use is a per-process `jti` guard — the TTL is the real boundary
