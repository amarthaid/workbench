...
export interface Swarm {
  ...
  state(): { ready: boolean; done: boolean; poseMix: number; count: number; targets: SwarmTarget[]; particles: SwarmTarget[]; bigCount: number; logoCount: number };
}
...
export function createSwarm(canvas: HTMLCanvasElement, opts: SwarmOptions = {}): Swarm {
  ...
  async function build() {
    ...
    if (useLogos) {
      ...
      for (const a of logoSprites) a.size = 40 + rnd() * 50;
      big = [];
    } else {
      logoSprites = [];
  ...
  function draw(t: number) {
    ...
    for (const a of logoSprites) {
      ...
      const alpha = Math.min(0.95, 0.35 + (a.z + 1) * 0.3);
  ...
  return {
    ...
    replay() { void build(); },
    state() { return { ready, done, poseMix, count: parts.length, targets: targets.slice(), particles: parts.map(({ tx, ty, tz, rim, shape }) => ({ tx, ty, tz, rim, shape })), bigCount: big.length, logoCount: logoSprites.length }; },
  };
}
